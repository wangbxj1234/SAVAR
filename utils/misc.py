...
def init_distributed_mode(local_out_path, only_sync_master=False, timeout=30):
    try:
        ...
        print(f'[dist init/barrier] {e!r}', flush=True)
        time.sleep(10)
...
class SyncPrint(object):
    def __init__(self, local_output_dir, sync_stdout=True):
        ...
        self._same_output_target = False
        try:
            term_stat = os.fstat(self.terminal_stream.fileno())
            file_stat = os.fstat(self.file_stream.fileno())
            self._same_output_target = (
                term_stat.st_dev == file_stat.st_dev and
                term_stat.st_ino == file_stat.st_ino
            )
        except Exception:
            self._same_output_target = False
        if existing:
    ...
    def write(self, message):
        ...
        # If stdout/stderr is already redirected to the same file,
        # avoid writing twice to prevent duplicated/interleaved logs.
        if not self._same_output_target:
            self.file_stream.write(message)
        if '\n' in message and not self._same_output_target:
            self.file_stream.flush()
...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        """
        # In non-distributed or single-process mode, do nothing.
        if not dist.initialized() or not tdist.is_available() or not tdist.is_initialized():
            return
        # In distributed mode, aggregate count and total across all ranks.
...
class MetricLogger(object):
    ...
    def log_every(self, start_it, max_iters, itrt, print_freq, header=None):
        ...
        if isinstance(itrt, Iterator) and not hasattr(itrt, 'preload') and not hasattr(itrt, 'set_epoch'):
            ...
            for i, obj in enumerate(itrt, start=start_it):
                self.data_time.update(time.time() - self.iter_end_t)
                yield i, obj
                self.iter_time.update(time.time() - self.iter_end_t)
                if i in self.log_iters:
...
def auto_resume(args: arg_util.Args, pattern='ckpt*.pth') -> Tuple[List[str], int, int, dict, dict]:
    ...
    if len(all_ckpt) == 0:
        ...
        # 严格选择时间戳最新的文件（无论是 last.pth 还是 iter.pth）
        selected_ckpt = all_ckpt[0]
        
        info.append(f'[auto_resume] load ckpt from @ {selected_ckpt} ...')
        ckpt = torch.load(selected_ckpt, map_location='cpu')
        ep, it = ckpt.get('epoch', 0), ckpt.get('iter', 0)

        # NOTE:
        # - epoch ckpt (ar-ckpt-ep*, last, best): saved with {'epoch': ep+1, 'iter': 0}
        # - iter  ckpt (ar-ckpt-iterXXXXX): saved with {'epoch': ep, 'iter': g_it+1} (global it)
        # For iter-ckpt, we can't directly use global 'iter' as start_it (train_one_ep expects in-epoch it).
        # Luckily we also save args.cur_it = f'{local_it+1}/{iters_train}', so we parse that.
        start_ep, start_it = int(ep), int(it)
        if int(it) != 0:
            try:
                args_state = ckpt.get('args', {}) or {}
                cur_it = args_state.get('cur_it', None)
                if isinstance(cur_it, str) and '/' in cur_it:
                    num_s, den_s = cur_it.split('/', 1)
                    local_next_it = int(num_s.strip())  # next local it index (0-based) == it+1
                    iters_train = int(den_s.strip())
                    if local_next_it >= iters_train:
                        start_ep = int(ep) + 1
                        start_it = 0
                    else:
                        start_ep = int(ep)
                        start_it = local_next_it
                    info.append(f'[auto_resume] parsed cur_it={cur_it} => start_ep={start_ep}, start_it={start_it}')
                else:
                    info.append(f'[auto_resume] ⚠️ iter-ckpt but missing/invalid args.cur_it={cur_it!r}; fallback start_it=0')
                    start_ep = int(ep)
                    start_it = 0
            except Exception as e:
                info.append(f'[auto_resume] ⚠️ failed to parse iter-ckpt cur_it; fallback start_it=0, err={e}')
                start_ep = int(ep)
                start_it = 0

        info.append(f'[auto_resume success] resume from ep{start_ep}, it{start_it}')
        return info, start_ep, start_it, ckpt.get('trainer', {}), ckpt.get('args', {})
