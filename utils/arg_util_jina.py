"""
arg_util 的副本，仅给 train_condition_jina_1024.py 使用；
在 Args 中增加 precompute_jina_emb，避免改动主训练的 arg_util。
"""
import json
import os
import random
...
class Args(Tap):
    data_path: str = '/home/ywxzml3j/ywxzml3juser37/sparse_attention/Datasets/ILSVRC/imagenet-1k/ILSVRC___imagenet-1k/default/0.0.0/49e2ee26f3810fb5a7536bbf732a7b07389a47b5'
    exp_name: str = 'text'
    ...
    # 若指定（例如 128），则 tlr = ac * tblr * 该值 / 256，而实际训练仍用 --bs 对应的 glb_batch_size
    tlr_lin_glb_batch: Optional[int] = None
    ac: int = 1
    ...
    hflip: bool = False
    workers: int = 8
    precompute_jina_emb: int = 0  # 1: only precompute Jina-CLIP-v2 image embeddings and exit
    max_iters_per_ep: int = 0  # 0 means full epoch; >0 caps train iterations for quick ablation
    iter_ckpt_interval: int = 1000  # save ar-ckpt-last.pth every N global iterations; <=0 disables iter saves
    fhat_k: int = 4  # VAE multi-scale index for img_to_fhat_k (condition spatial tokens)
    k: Optional[int] = None  # alias for --fhat_k; if set, overrides fhat_k after parse
    # 从其它 run 的 condalign-*.pth 只加载 model（与可选 loss_fn），优化器从零，epoch/it 从 0 起。
    # 若输出目录存在 condalign-ckpt-last.pth 会走完整 resume（含优化器与 epoch），init 不生效；只想 warm-start 时请删掉 last。
    condalign_init_ckpt: str = ''
    ...
    commit_msg: str = (['[unknown]'])[-1].strip()
    acc_mean: float = None
    acc_tail: float = None
    L_mean: float = None
    L_tail: float = None
...
def init_dist_and_get_args():
    ...
    _lr_bs = args.glb_batch_size if args.tlr_lin_glb_batch is None else args.tlr_lin_glb_batch
    args.tlr = args.ac * args.tblr * _lr_bs / 256
