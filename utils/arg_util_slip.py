"""
arg_util 的副本，仅给 train_condition_slip.py 使用；
在 Args 中增加 precompute_slip_emb，避免改动主训练的 arg_util。
"""
import json
import os
import random
...
class Args(Tap):
    data_path: str = '/home/ywxzml3j/ywxzml3juser37/sparse_attention/Datasets/ILSVRC/imagenet-1k/ILSVRC___imagenet-1k/default/0.0.0/49e2ee26f3810fb5a7536bbf732a7b07389a47b5'   # real ImageNet image path
    exp_name: str = 'text'
    ...
    lr_ratio: float = 1.0   # multiply lr after resume (no reset of steps)
    ...
    ep: int = 250
    wp: float = 0           # auto set to ep*1/50 if =0 (official default)
    wp0: float = 0.005      # initial lr ratio at the begging of lr warm up (official default)
    wpe: float = 0.01       # final lr ratio at the end of training (official default)
    ...
    # other hps
    saln: bool = True       # whether to use shared adaln
    ...
    ckpt: bool = False      # whether to use activation checkpointing

    # init / resume
    init_d36: bool = False  # when training from scratch (no resume ckpt), load official var_d36 weights if available

    # data
    pn: str = '1_2_3_4_5_6_8_10_13_16'
    patch_size: int = 16
    patch_nums: tuple = None    # [automatically set; don't specify this]
    ...
    precompute_slip_emb: int = 0  # 1: only precompute SigLIP image embeddings and exit (train_condition_slip.py)
    ...
    commit_msg: str = ( ['[unknown]'])[-1].strip()    # [automatically set; don't specify this]
