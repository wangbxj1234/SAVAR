...
class Args(Tap):
    ...
    def dump_log(self):
        if not dist.is_local_master():
            return
        # Only write header for the very first epoch (ep == 1), not any epoch containing '1/' like 11, 12, 21, etc.
        if self.cur_ep.split('/')[0] == '1': # first time to dump log (epoch 1 only)
