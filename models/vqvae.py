...
class VQVAE(nn.Module):
    ...
    def img_to_fhat_k(self, inp_img_no_grad: torch.Tensor,k: int, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[torch.LongTensor]:    # return List[Bl]
        f = self.quant_conv(self.encoder(inp_img_no_grad))
        return self.quantize.f_to_idxBl_or_fhat(f, to_fhat=True, v_patch_nums=v_patch_nums)[k]#self.quantize.f_to_idxBl_or_fhat_k(f,k, to_fhat=True, v_patch_nums=v_patch_nums)

    def img_to_idxBl_and_fhat_k(self, inp_img_no_grad: torch.Tensor, k: int, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> Tuple[List[torch.LongTensor], torch.Tensor]:
        """Single encoder pass returning both idx lists and fhat at scale k."""
        f = self.quant_conv(self.encoder(inp_img_no_grad))
        idx_Bl_list, fhat_list = self.quantize.f_to_idxBl_and_fhat(f, v_patch_nums=v_patch_nums)
        return idx_Bl_list, fhat_list[k]
