...
class VectorQuantizer2(nn.Module):
    ...
    def f_to_idxBl_and_fhat(self, f_BChw: torch.Tensor, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> Tuple[List[torch.LongTensor], List[torch.Tensor]]:
        """Single-pass quantization that returns both idx lists and fhat lists."""
        B, C, H, W = f_BChw.shape
        f_no_grad = f_BChw.detach()
        f_rest = f_no_grad.clone()
        f_hat = torch.zeros_like(f_rest)
        
        idx_Bl_list: List[torch.Tensor] = []
        fhat_list: List[torch.Tensor] = []
        ...
        for si, (ph, pw) in enumerate(patch_hws):
            ...
            f_hat.add_(h_BChw)
            f_rest.sub_(h_BChw)
            idx_Bl_list.append(idx_N.reshape(B, ph*pw))
            fhat_list.append(f_hat.clone())
        
        return idx_Bl_list, fhat_list
