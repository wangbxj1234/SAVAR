...
class VAR(nn.Module):
    def __init__(
            ...
            n_cond_embed=768, depth=16, embed_dim=1024, num_heads=16, mlp_ratio=4., drop_rate=0., attn_drop_rate=0.,
            drop_path_rate=0.,
        ...
        # Calculate fhat_k spatial size: max(patch_nums) x max(patch_nums)
        # because input resolution = max(patch_nums) * patch_size (16), 
        # and VAE encoder downsamples by 16x, so fhat_k spatial = max(patch_nums) x max(patch_nums)
        max_patch_num = max(patch_nums)
        fhat_k_spatial_size = max_patch_num

        self.down1 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # [B,64,H,H] where H = fhat_k_spatial_size
            nn.ReLU(),
            nn.Conv2d(64, 96, kernel_size=3, stride=1, padding=1),  # [B,96,H,H]
            nn.ReLU()
        )
        self.proj1 = nn.Sequential(
            nn.Flatten(),  # [B,96*H*H]
            nn.Linear(96 * fhat_k_spatial_size * fhat_k_spatial_size, 768),  # 全连接映射到 [B,768]
            nn.LayerNorm(768)
        )
        ...
        # Gate for blending sos source:
        # alpha=1.0 -> original behavior (pure fhat->sos),
        # alpha=0.0 -> use cond branch as sos anchor.
        self.sos_mix_alpha = float(os.getenv("FIXK_SOS_MIX_ALPHA", "1.0"))
        ...
        self.cond_proj2 = nn.Linear(n_cond_embed, self.C)
    ...
    def autoregressive_infer_cfg(
        ...
        fhat_embeds = self.proj1(self.down1(fhat_k))
        sos_fhat = self.cond_proj2(torch.cat((fhat_embeds, noise), dim=0))
        cond_BD = self.cond_proj(torch.cat((label_B, noise), dim=0))
        if self.sos_mix_alpha == 1.0:
            sos = sos_fhat
        elif self.sos_mix_alpha == 0.0:
            sos = cond_BD
        else:
            sos = self.sos_mix_alpha * sos_fhat + (1.0 - self.sos_mix_alpha) * cond_BD
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            # last_L = cur_L
            cur_L += pn * pn
            cond_BD_or_gss = self.shared_ada_lin(cond_BD)  # 就是identity
            x = next_token_map
    ...
    def forward(self, embed, fhat_k: torch.Tensor,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            sos_fhat = self.cond_proj2(embed2)
            if self.sos_mix_alpha == 1.0:
                sos_src = sos_fhat
            elif self.sos_mix_alpha == 0.0:
                sos_src = embed
            else:
                sos_src = self.sos_mix_alpha * sos_fhat + (1.0 - self.sos_mix_alpha) * embed

            sos = sos_src.unsqueeze(1).expand(B, self.first_l, -1) + self.pos_start.expand(B, self.first_l,
                                                                                         -1)  # (B, 1, C)

            # if self.prog_si == 0: x_BLC = sos 都负1了还ifnmif
